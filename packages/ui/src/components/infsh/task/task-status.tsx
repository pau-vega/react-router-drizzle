...
export function extractTaskEventTimes(task: Task): TaskEventTimes {
  const findEventTime = (status: TaskStatus) => task.events?.find((e: TaskEvent) => e.status === status)?.event_time
...
export const StatusPillSimple = memo(function StatusPillSimple({ status, className }: StatusPillSimpleProps) {
  return (
    <div className={cn("px-3 py-1.5 rounded-full text-xs font-medium", getStatusColor(status), className)}>
