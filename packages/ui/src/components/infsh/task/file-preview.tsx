      ...
      case "image":
        return (
          <img
            ...
            className={cn("rounded-md w-full h-full", objectFit === "cover" ? "object-cover" : "object-contain")}
            onLoad={onLoad}
            onError={() => onError?.()}
          />
        )
    ...
    >
      ...
      {buttons && (
        <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-10">
          <Button onClick={handleCopyUrl} variant="outline" size="icon" className="h-8 w-8" title="Copy URL">
          ...
          <Button onClick={handleOpenExternal} variant="outline" size="icon" className="h-8 w-8" title="Download">
...
function TextPreview({ url, onLoad, onError }: { url: string; onLoad?: () => void; onError?: () => void }) {
