...
import { ChatContainer } from "@ui/components/infsh/agent/chat-container"
import { ChatMessages } from "@ui/components/infsh/agent/chat-messages"
import { ChatInput } from "@ui/components/infsh/agent/chat-input"
import { MessageBubble } from "@ui/components/infsh/agent/message-bubble"
import { MessageContent } from "@ui/components/infsh/agent/message-content"
import { MessageReasoning } from "@ui/components/infsh/agent/message-reasoning"
import { MessageStatusIndicator } from "@ui/components/infsh/agent/message-status-indicator"
import { ToolInvocations } from "@ui/components/infsh/agent/tool-invocations"
...
function isTerminalStatus(status: string | undefined): boolean {
  return (
    status === ChatMessageStatusReady || status === ChatMessageStatusFailed || status === ChatMessageStatusCancelled
  )
}
...
const EmptyState = memo(function EmptyState({
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center text-center p-6 text-muted-foreground">
      ...
      {examplePrompts.length > 0 && <ExamplePrompts prompts={examplePrompts} onSelect={sendMessage} />}
    </div>
  )
})

const MessageRow = memo(function MessageRow({ message, isLast }: { message: ChatMessageDTO; isLast: boolean }) {
  ...
  return (
    <MessageBubble message={message}>
      {isAssistant && reasoningContent && (
        <MessageReasoning reasoning={reasoningContent} isReasoning={isGenerating && !hasTextContent(message)} />
      )}
  ...
  return (
    <div className="space-y-4 p-4">
      {messages.map((message, index) => (
        <MessageRow key={message.id} message={message} isLast={index === messages.length - 1} />
      ))}
...
export function Agent({
  ...
  return (
    <AgentChatProvider client={client} agentConfig={config} chatId={chatId} onChatCreated={onChatCreated}>
      <AgentContent
        className={className}
        compact={compact}
