...
import { TaskLogTypeRun, TaskLogTypeServe, TaskLogTypeSetup, TaskLogTypeTask } from "@inferencesh/sdk"
...
function getMergedLogsContent(logs: Record<TaskLogType, TaskLog | undefined>, onlyLastLine = false): string {
  let mergedContent = ""
...
function getHighestAvailableLogType(logs: Record<TaskLogType, TaskLog | undefined>): TaskLogType {
...
export const TaskLogs = memo(function TaskLogs({ task, className }: TaskLogsProps) {
  ...
  const [selectedTab, setSelectedTab] = useState<TaskLogType>(() => getHighestAvailableLogType(logs))
    ...
    if (logs) {
      ...
      if (!logs[selectedTab]) {
        ...
        if (availableLogs.length > 0) {
          ...
          const nextAvailable = LOG_TYPES.slice(currentIndex + 1).find((type) => logs[type.key] !== undefined)
...
export const LogViewer = memo(function LogViewer({ content }: LogViewerProps) {
  ...
  return (
    <div className="relative">
      <div ref={containerRef} className="max-h-[400px] bg-muted/50 rounded-lg text-[12px] p-4 overflow-y-auto">
        <pre
        ...
        >
          {lines.map((line: string, i: number) => (
            <div key={i} className="flex m-0 p-0">
              ...
              <span className="flex-1 whitespace-pre text-muted-foreground">{line}</span>
            </div>
          ))}
        </pre>
      </div>
