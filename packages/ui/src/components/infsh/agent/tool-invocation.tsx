...
import { WidgetRenderer } from "@ui/components/infsh/agent/widget-renderer"
import { parseWidget, type WidgetAction, type WidgetFormData } from "@ui/components/infsh/agent/widget-types"
import { TaskOutputWrapper } from "@ui/components/infsh/task/task-output-wrapper"
...
const FinishBlock = memo(function FinishBlock({
  ...
  const resultMessage =
    finish?.result && typeof finish.result === "string" && !isActive ? finish.result.toLowerCase() : null
  ...
  return (
    <div className="my-6 w-full">
      ...
      {resultMessage && (
        <p className="text-xs text-muted-foreground/40 text-center mt-2 break-words">{resultMessage}</p>
      )}
    </div>
  )
})
    ...
    return (
      <div className={cn("flex flex-col items-start", className)}>
        <Collapsible
          ...
          className={cn("group w-full overflow-hidden rounded text-muted-foreground", isOpen && "border bg-muted/10")}
        >
