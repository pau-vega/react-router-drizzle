...
export const MessageReasoning = memo(function MessageReasoning({
  ...
  const preview =
    isReasoning && getLastLine() ? (
      <span className="text-xs text-muted-foreground/40 truncate max-w-[300px] lowercase ml-2">- {getLastLine()}</span>
    ) : null
  ...
  return (
    <div className={cn("flex flex-col items-start w-fit mb-2", className)}>
      <Collapsible
        ...
        className={cn("group w-full overflow-hidden rounded-lg text-muted-foreground", isOpen && "border bg-muted/10")}
      >
        <div className="flex items-center p-2">
          ...
          >
            ...
            <span className={cn("relative text-sm", isReasoning && "animate-pulse")}>{label}</span>
            {!isOpen && preview}
          </CollapsibleTrigger>
        </div>
        <CollapsibleContent>
