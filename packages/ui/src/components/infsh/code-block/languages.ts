import type { LanguageDefinition } from "@ui/components/infsh/code-block/types"
...
export const javascript: LanguageDefinition = {
  ...
  keywords: {
    ...
    other: new Set(["new", "delete", "typeof", "instanceof", "void", "this", "super", "satisfies", "keyof", "infer"]),
    type: new Set(["string", "number", "boolean", "object", "symbol", "bigint", "any", "unknown", "never", "void"]),
  },
  patterns: {
    comment: [
      /^\/\/[^\n]*/, // single line
...
export const css: LanguageDefinition = {
  ...
  keywords: {
    other: new Set(["important", "inherit", "initial", "unset", "none", "auto"]),
  },
  patterns: {
    comment: [/^\/\*[\s\S]*?\*\//],
    string: [/^"(?:[^"\\]|\\.)*"/, /^'(?:[^'\\]|\\.)*'/],
...
export const rust: LanguageDefinition = {
  ...
  keywords: {
    ...
    declaration: new Set(["fn", "let", "mut", "const", "static", "struct", "enum", "trait", "impl", "type", "where"]),
    control: new Set([
      "if",
      "else",
      "for",
